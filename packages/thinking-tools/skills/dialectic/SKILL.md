...
**CRITICAL: Both `subagent` tool calls MUST be in a single message for true parallel execution.** Do NOT launch sequentially. Use foreground agents (`background: false`) in the same message so they run in parallel and block until both complete.
...
```
/skill:dialectic this microservice split improves maintainability
```

**Bug analysis:**
```
/skill:dialectic the connection pool fixes the timeout issue
```

**Performance claims:**
```
/skill:dialectic caching reduced database load by 80%
```

**Refactoring safety:**
```
/skill:dialectic extracting this interface simplifies testing
```

**Code review:**
```
/skill:dialectic this implementation is thread-safe
```

**Review changes:**
```
/skill:dialectic review the changes in server.go
```
