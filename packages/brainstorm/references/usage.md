...
Invoke directly with `/skill:brainstorm` or let it activate via intent matching.
...
### Phase 4: Next Steps
- **Write plan** — creates a plan file under `docs/plans/` with brainstorm context
...
→ Phase 4: user picks "Write plan" → creates a plan file with full context
...
→ asks what rules to add, writes to `~/.pi/agent/brainstorm-rules.md`
```
